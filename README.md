...
This repository contains custom tasks and workflows for speedmap-trend. Follow the setup steps below to get started.


## Project Structure

- `src/ecoscope-workflows-ext-speedmap-trend`: Contains custom tasks
...
## Task Development
The tasks are defined under `src/ecoscope-workflows-ext-speedmap-trend/ecoscope_workflows_ext_speedmap_trend/tasks`. Here's an example of a task
...
You can also add other dependencies in `src/ecoscope-workflows-ext-speedmap-trend/pyproject.toml` under [tool.pixi.dependencies] section

Unittests are under `src/ecoscope-workflows-ext-speedmap-trend/tests` and can be tested it by
```bash
pixi run pytest-310
```
...
2. Run
   ...
   ecoscope-workflows-ext-speedmap-trend = { version = "*", channel = "file:///tmp/ecoscope-workflows-custom/release/artifacts/" }
   ```
...
2. Run the following command to compile your workflow:
   ...
   This will generate a folder called `ecoscope-workflows-speedmap-workflow` with your compiled workflow.
...
3. Test your workflow. First set up your output directory by
   ```bash
   mkdir -p /tmp/workflows/speedmap-trend/speedmap/output
   export ECOSCOPE_WORKFLOWS_RESULTS=file:///tmp/workflows/speedmap-trend/speedmap/output
   ```
   Then run your workflow by
   ```bash
   cd workflows/speedmap/ecoscope-workflows-speedmap-workflow
   pixi run ecoscope-workflows-speedmap-workflow run --config-file ../param.yaml --execution-mode sequential --mock-io
   ```
   You can find the results in your output folder in `/tmp/workflows/speedmap-trend/speedmap/output/result.json`
...
5. Update the version in pixi.toml
   ```toml
   [dependencies.ecoscope-workflows-ext-speedmap-trend]
